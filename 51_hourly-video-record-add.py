    ...
    def run(self):
        ...
        if self.time_label == '03:00' and get_week_day() == 5:
            ...
            try:
                ...
                # TODO remove try after validation
                try:
                    ...
                    self.logger.warning('fail_aids: %r' % fail_aids)
                    sc_send('NEW_FEATURE_RUNNING_FEEDBACK_ADD_C0_BASE_SUCCESS', '%d c0 no base videos, %d added, %d aids fail, fail aids: %s'
                            % (len(no_base_c0_video_aid_list), len(success_aids), len(fail_aids), str(fail_aids)))
                except Exception as e:
                    sc_send('NEW_FEATURE_RUNNING_FEEDBACK_ADD_C0_BASE_FAIL', str(e))
